...
A web-based tool built with Flask that allows users to:
...
This tool is intended for educational purposes and personal use only. Users are responsible for complying with YouTube's Terms of Service and copyright laws in their jurisdiction.
...
- Flask
- yt-dlp
- ffmpeg (recommended for better audio quality)
...
1. Enter a YouTube video URL or multiple URLs (one per line)
2. Click "Download" to get the MP3 file(s)
3. To search for videos, use the search box at the top
4. Use "Download All Files" to get all previously downloaded files as a ZIP
